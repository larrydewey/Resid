...
- **Status**: lexer 7, parser 10, resid-type 33, resid-codegen 4 — **63 tests pass**. `resid-ir` implements the spec-§6 primitive numeric types and mixed-width widening. `resid-type` has 33 tests covering literal inference, widening, signed/unsigned mixing rejection, bitwise/float errors, cast, if, RT, and `check_program`. `resid-codegen` emits verified LLVM IR: functions, arithmetic, casts, calls, bool. The full workspace compiles against system LLVM 22 (inkwell `0.9` / `llvm22-1`). `residc <f> emit-ir` runs the full pipeline (lex → parse → type check → LLVM IR).
