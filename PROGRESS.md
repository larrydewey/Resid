...
- **Tests**: 495 pass (lexer 17, parser 90, resid-ir 46, resid-type 181,
  resid-codegen 130, residc 31 e2e).
...
| 5. Stdlib, Build System | Partial | `resid-builtin`/`resid-build` stubs; compile clean. Runtime helpers landed: conversion helpers, checked/wrapping/saturating arithmetic, ranges/slicing, raw strings, byte strings, `#location`, f-string interpolation, runtime `Str + Str` concat, Dec(N) exact decimal arithmetic/display/conversions, `filesystem.write_all` (M6 P1), spawn with capture loading and sum-type casting (M6 P2). Still missing: handles, `resid-build` crate, full `resid-builtin` stdlib. |
...
*Last updated: 2026-08-19 — M6 P2 (a)(b)(c)(d) `Result(T, RegionError)` + spawn done (495 tests); `load_capture` reads raw capture array via byte-offset GEP; `lower_block_with_tail` returns 3-tuple for spawn return semantics; sum-type casting via payload read+rebuild; 3 new e2e spawn tests pass.*