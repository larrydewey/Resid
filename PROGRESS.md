...
- **Stage-2 constraint types parity**: constraint type aliases
  (`Int[value > 0]`, `Int where value >= 0`) implemented in Rust pipeline
  with discharge on constant bindings; Stage-2 driver merge blocked by
  `tools/merge_driver.py` header-extraction bug (tracked separately).
