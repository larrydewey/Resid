...
- **469 tests total**: lexer 17, parser 88, resid-ir 46, resid-type 171, resid-codegen 122,
...
| Visibility rules (§22) | ❌ |
| Structured spawn (§19) | ❌ |
| Full numeric family (§6, §32) | Partial — Int8..Int64, UInt8..UInt64, Float16/32/64/128, plus Int(128)/UInt(128) literals/arithmetic/casts and `Int128ToString`/`UInt128ToString`, Int(256)/Int(512) arithmetic + `Int256ToString`/`UInt512ToString` (limb-decomposed), literals of any width up to 512 bits (digits carried through lexing; overflow rejected per §6). Dec(N) type system done (spec v3.1 §6.6a): type, `m`-suffix literals, `dN`/`iN`/`uN`/`fN` helpers; runtime arithmetic pending |
