...
- **448 tests total**: lexer 14, parser 88, resid-ir 41, resid-type 161, resid-codegen 120,
  residc 24 (e2e). Destructuring, `_ = expr` discard,
...
- **Self-host roadmap**: detailed in §12. Conversion helpers (§12.1, task 5.1),
  ...
  per spec §6 and widens inference for huge literals). Next: remaining wide
  numerics (wide Float types), handles (§12.1, task 5.7), spawn
...
| Visibility rules (§22) | ❌ |
| Structured spawn (§19) | ❌ |
| Full numeric family (§6, §32) | Partial — Int8..Int64, UInt8..UInt64, Float16/32/64, plus Int(128)/UInt(128) literals/arithmetic/casts and `Int128ToString`/`UInt128ToString`, Int(256)/Int(512) arithmetic + `Int256ToString`/`UInt512ToString` (limb-decomposed), literals of any width up to 512 bits (digits carried through lexing; overflow rejected per §6) |
