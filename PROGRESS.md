...
- **441 tests total**: lexer 13, parser 88, resid-ir 41, resid-type 158, resid-codegen 118,
  residc 23 (e2e). Destructuring, `_ = expr` discard,
...
- **Self-host roadmap**: detailed in §12. Conversion helpers (§12.1, task 5.1),
  ...
  done. Next: remaining wide numerics (256/512), handles (§12.1, task 5.7), spawn
  (§12.1, task 5.8), or the `resid-build` crate (§12.1, task 5.10).
...
| **5.9** | Wide numeric runtime (Int256, Int512, Float256, Float512) | 🚧 In progress | — | Software emulation: `[u64; N]` arrays + runtime lib (add, mul, cmp). **Int(128)/UInt(128) done**: `i128` LLVM lowering, wide literal construction via `const_int_from_string` (magnitude-derived holding width when no target), `Int128ToString`/`UInt128ToString` decimal printers in `resid_rt.c`. **Int(256)/Int(512) arithmetic done** via LLVM arbitrary-width integers (`custom_width_int_type`); `Int256ToString`/`UInt256ToString`/`Int512ToString`/`UInt512ToString` stringify by decomposing the value into little-endian u64 limbs (C ABI has no native 256-bit type). Remaining: wide Float types, literals > u128 (lexer caps at u128 today). |
...
| Visibility rules (§22) | ❌ |
| Structured spawn (§19) | ❌ |
| Full numeric family (§6, §32) | Partial — Int8..Int64, UInt8..UInt64, Float16/32/64, plus Int(128)/UInt(128) literals/arithmetic/casts and `Int128ToString`/`UInt128ToString`, Int(256)/Int(512) arithmetic + `Int256ToString`/`UInt512ToString` (limb-decomposed) |
...
| Float arithmetic (§6.2) | ✅ | fadd/fsub/fmul/fdiv; Float(16/32/64). |
