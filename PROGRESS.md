...
- **437 tests total**: lexer 13, parser 88, resid-ir 41, resid-type 156, resid-codegen 117,
  residc 22 (e2e). Destructuring, `_ = expr` discard,
...
| **5.9** | Wide numeric runtime (Int256, Int512, Float256, Float512) | 🚧 In progress | — | Software emulation: `[u64; N]` arrays + runtime lib (add, mul, cmp). **Int(128)/UInt(128) subset done**: `i128` LLVM lowering, wide literal construction via `const_int_from_string` (magnitude-derived holding width when no target), `Int128ToString`/`UInt128ToString` decimal printers in `resid_rt.c`. Remaining: 256/512-bit emulation, Float wide types. |
