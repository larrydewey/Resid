# pidigits — Rust, best track

Source: https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/pidigits-rust-4.html
(the fastest Rust program on the Benchmarks Game pidigits page at time of
adaptation, 0.71s there). Benchmarks Game programs are distributed under
...
Program: single-threaded spigot using `rug` (GMP) in-place operations.
...
Dependencies: rug 1.30.0 + gmp-mpfr-sys 1.7.1 with `use-system-libs` (links host libgmp; only the `integer` feature), lockfile.
