#!/usr/bin/env python3
"""Regenerate examples/driver.res from examples/codegen.res + examples/typecheck.res.

Recipe (M6 stage-2):
  base   = codegen.res minus CLI bits (pick_out, Int main)
  chunk  = typecheck.res from its Environment banner onward, minus Int main,
  ...
  tail   = existing driver.res driver section (pick_opt + main) with the
           declare-header list refreshed from codegen.res's main
"""
import re, sys
...
def main():
    cg = read('examples/codegen.res')
    tc = read('examples/typecheck.res')
    dv = read('examples/driver.res')
    ...
    # 3. tail: driver section from old driver.res, header refreshed
    ds = next(i for i, l in enumerate(dv) if '─── Driver:' in l)
    tail = dv[ds:]
    # refresh header list inside main using codegen.res's current header entry
    ...
    banner = ['', '// ═══════════════════════════════════════════════════════════════',
              '// Checker stage — fused from typecheck.res (ck_-prefixed where', '// colliding with the codegen stage above). Regenerated by',
              '// tools/merge_driver.py — do not edit by hand.', '// ═══════════════════════════════════════════════════════════════', '']

    out = base + banner + chunk_t + [''] + tail
    open(f'{ROOT}/examples/driver.res', 'w').write('\n'.join(out) + '\n')
    print(f"wrote examples/driver.res ({len(out)} lines)")
